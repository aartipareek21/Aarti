			...
			case '/content_view':
				...
				if(isset($_REQUEST['btn_course_content']))
				{
					...
					while($fetch=$res->fetch_object()) // fetch all data
					{
					 ...
					 $res_videos = $this->select_where('course_videos', $where);
        $arr_videos = [];
        while ($fetch = $res_videos->fetch_object()) {
            $arr_videos[] = $fetch;
        }
				}
			...
			break;
				...
				case '/topic_view':
					$arr_topic = []; // Ensure it's always an array
				
					if (isset($_REQUEST['btn_course_topics'])) {
						$course_id = $_REQUEST['btn_course_topics'];
						$where = array("course_id" => $course_id);
						$res = $this->select_where('course_topics', $where);
